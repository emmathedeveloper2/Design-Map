...
const processDefaults = (record: any, options: CreateTableOptions) => {
  ...
  Object.keys(options).forEach((key) => {
    if (options[key].default === "autogenerate" && record[key] === undefined) {
      ...
      if (options[key].type === "string")
        updatedRecord[key] = generateStringId();
    }

    if (options[key].default === "now" && record[key] === undefined) {
      if (options[key].type === "number")
        updatedRecord[key] = new Date().getTime();

      if (options[key].type === "string")
        updatedRecord[key] = new Date().toISOString();
    }
...
function createTable<T>(name: string, options: string) {
  ...
  function addOne(record: T): Promise<number> {
     ...
     */
    const connection: IDBOpenDBRequest = indexedDB.open(
      DATABASE_NAME,
      DATABASE_VERSION
    );
      ...
      connection.addEventListener("success", (e) => {
        ...
        operation.addEventListener("success", (e) => {
          ...
          const match = data.find((record) =>
            Object.entries(query as any).every(
...
function initializeDatabase() {
  ...
  connection.addEventListener("upgradeneeded", (e) => {
    ...
    const Project_table: IDBObjectStore = db.createObjectStore("project", {
      keyPath: "id",
    });

    const Stage_table: IDBObjectStore = db.createObjectStore("stage", {
      keyPath: "id",
    });

    const Task_table: IDBObjectStore = db.createObjectStore("task", {
      keyPath: "id",
    });
  });
...
const Project_table = createTable<Project>(
  "project",
  '{"id":{"name":"id","type":"number","default":"autogenerate"},"createdAt":{"name":"createdAt","type":"number","default":"now"}}'
);
const Stage_table = createTable<Stage>(
  "stage",
  '{"id":{"name":"id","type":"number","default":"autogenerate"},"createdAt":{"name":"createdAt","type":"number","default":"now"}}'
);
const Task_table = createTable<Task>(
  "task",
  '{"id":{"name":"id","type":"number","default":"autogenerate"},"createdAt":{"name":"createdAt","type":"number","default":"now"}}'
);
